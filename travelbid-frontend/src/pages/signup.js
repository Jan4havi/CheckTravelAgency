...
export default function Signup() {
  ...
  // ── OTP state (UI-only flow — backend handles real verification) ───────────
  ...
  // ── OTP helpers (demo — in prod replace with SMS API call) ────────────────
  const sendOtp = () => {
    if (!validatePhone(formData.phone)) {
      setErrors({ ...errors, phone: 'Enter valid 10-digit mobile number first' });
      return;
    }
  ...
  // ── Form submit → AuthContext.signup → POST /api/v1/auth/signup/traveler ──
        ...
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          {errors.general === 'ALREADY_REGISTERED' && (
            <div style={{ background: '#fff7ed', border: `1.5px solid ${T.accent}`, padding: '14px', borderRadius: '12px', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => navigate('/login')} style={{ flex: 1, padding: '8px', background: T.primary, color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Traveler Sign In</button>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontSize: '12px', fontWeight: '600', color: T.textMid }}>Full Name *</label>
              <div style={{ position: 'relative' }}>
                <MdPerson size={17} color={T.textLight} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)' }} />
                <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} placeholder="Rahul Sharma" required
                  style={inp(errors.full_name)}
                  onFocus={e => e.target.style.borderColor = T.primary} onBlur={e => e.target.style.borderColor = errors.full_name ? '#ef4444' : T.border} />
              </div>
            ...
            {/* Email */}
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontSize: '12px', fontWeight: '600', color: T.textMid }}>Email Address *</label>
              <div style={{ position: 'relative' }}>
                <MdEmail size={17} color={T.textLight} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)' }} />
            ...
            {/* Phone + OTP */}
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontSize: '12px', fontWeight: '600', color: T.textMid }}>Phone Number *</label>
              <div style={{ display: 'flex', gap: '8px' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  <MdPhone size={17} color={T.textLight} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)' }} />
                  <input type="tel" name="phone" value={formData.phone}
                    onChange={e => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) handleChange({ target: { name: 'phone', value: v } }); }}
                    placeholder="9876543210" maxLength="10" required
                    style={{ ...inp(errors.phone), paddingRight: otpVerified ? '44px' : undefined }}
              ...
              {otpVerified && <p style={{ color: '#10b981', fontSize: '12px', marginTop: '4px', fontWeight: '600' }}>Phone verified!</p>}
              ...
              {otpStep && !otpVerified && (
                <div style={{ marginTop: '12px', background: `${T.primary}06`, border: `1.5px solid ${T.primary}30`, borderRadius: '12px', padding: '16px' }}>
                  ...
                  <div style={{ marginTop: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <p style={{ fontSize: '11px', color: T.textLight }}>
                      Didn't receive?{' '}
                      {resendTimer > 0
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPass ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} placeholder="Create strong password" required
              ...
              {errors.password
                ...
                : <p style={{ fontSize: '11px', color: T.textLight, marginTop: '4px' }}>Min 8 chars, uppercase, lowercase, number & special char</p>}
            </div>
          ...
          <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: `1px solid ${T.border}`, textAlign: 'center' }}>
            <p style={{ fontSize: '14px', color: T.textMid, marginBottom: '8px' }}>Already have an account? <Link to="/login" style={{ color: T.primary, fontWeight: '700', textDecoration: 'none' }}>Traveler sign in</Link></p>
            <p style={{ fontSize: '14px', color: T.textMid }}>Travel agency? <Link to="/agency-login" style={{ color: T.primary, fontWeight: '700', textDecoration: 'none' }}>Agency login</Link> or <Link to="/agency-signup" style={{ color: T.primary, fontWeight: '700', textDecoration: 'none' }}>Register here</Link></p>
          </div>
        </div>
      </div>
