...
import { tripsAPI, usersAPI, getErrorMessage } from '../services/api';
...
export default function Dashboard() {
  ...
  const [myActiveTrip, setMyActiveTrip] = useState(null);
  ...
  useEffect(() => { checkProfileComplete(); }, [user]); // eslint-disable-line

  // ── Profile completeness check via /api/v1/auth/me ──────────────────────────
  const checkProfileComplete = async () => {
    if (!user) return;
    try {
      if (isAgency) {
        // Agency: check required fields from user object (populated from backend)
        const complete = user.phone && user.address && user.website &&
          user.gst_number && user.pan_number;
        setProfileComplete(!!complete);
      } else {
        // Traveler: phone + email
        const complete = user.phone && user.email;
        setProfileComplete(!!complete);
      }
  ...
  // ── Load all live trips for agencies ─────────────────────────────────────
  const loadAllTrips = async () => {
    setTripsLoading(true);
    try {
      const { data } = await tripsAPI.list({ status: 'Live', limit: 100 });
      setAllTrips(data.results || []);
    } catch (e) { console.error(e); } finally { setTripsLoading(false); }
  };

  // ── Load traveler's own active (Live) trip ────────────────────────────────
  const loadMyActiveTrip = async () => {
    try {
      const { data } = await tripsAPI.myTrips('Live');
      const trips = Array.isArray(data) ? data : [];
      const active = trips[0] || null;  // only 1 active trip allowed
      if (active) {
        setMyActiveTrip(active);
        setFormData({
          source: active.source || '',
          destination: active.destination || '',
          travelers: active.travelers?.toString() || '',
          startDate: active.start_date ? active.start_date.split('T')[0] : '',
          endDate: active.end_date ? active.end_date.split('T')[0] : '',
          tripScope: active.trip_scope || '',
          tripType: active.trip_type || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (myActiveTrip && editingTrip) {
        // PATCH — destination is locked (excluded from payload per API spec)
        const { destination: _dest, ...updatePayload } = payload;
        await tripsAPI.update(myActiveTrip.id, updatePayload);
      ...
      console.error(e);
      alert(getErrorMessage(e) || 'Failed to save trip.');
  ...
  const handleDeleteTrip = async () => {
    ...
    try {
      await tripsAPI.delete(myActiveTrip.id);
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '0 40px', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '13px' }}>
                        {myActiveTrip.destination} · {myActiveTrip.travelers} people · {myActiveTrip.start_date ? new Date(myActiveTrip.start_date).toLocaleDateString('en-IN') : 'N/A'}
                      </p>
                    </div>
                ...
                <div style={{ padding: '24px 32px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
                    {[
                      ...
                      { label: 'Dates', value: `${myActiveTrip.start_date ? new Date(myActiveTrip.start_date).toLocaleDateString('en-IN') : ''} → ${myActiveTrip.end_date ? new Date(myActiveTrip.end_date).toLocaleDateString('en-IN') : ''}` },
                ...
                <form onSubmit={handleSubmit} style={{ padding: '36px 40px' }}>
                  <div style={{ background: `${T.primary}06`, border: `1px solid ${T.primary}20`, borderRadius: '14px', padding: '16px 20px', marginBottom: '28px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                      {['Set a realistic budget range','Mention exact travel dates','Add number of adults & children','Specify hotel preference (budget/3★/5★)','Mention meals preference (veg/non-veg)','List any specific activities you want','Mention if flights are needed','Add any special requests (anniversary, etc.)'].map(tip => (
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    <label style={{ ...lbl, display: 'flex', alignItems: 'center', gap: '6px' }}><MdCurrencyRupee size={14} color={T.primary} /> Budget Range (Per Person) *</label>
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    <textarea value={formData.preferences} onChange={e => setFormData({ ...formData, preferences: e.target.value })}
                      placeholder="Hotel preference (budget/3★/5★), meals (veg/non-veg), activities, room type, flight needed, special occasions..." rows="6"
                  ...
                  {profileComplete === false && (
                    <div style={{ background: '#fff7ed', border: '1.5px solid #fed7aa', borderRadius: '12px', padding: '14px 18px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: '14px', fontWeight: '700', color: '#92400e', margin: 0, marginBottom: '4px' }}>Complete your profile first</p>
                        <p style={{ fontSize: '13px', color: '#b45309', margin: 0 }}>
                          Add your phone number and email to your profile before posting a trip.
                        </p>
                      </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                ...
                {(() => {
                  ...
                  const bidsLeft = totalBids; // backend tracks this — show plan limit for now
                  const pct = 100;
                  ...
                  return (
                    <div style={{ background: 'white', padding: '16px 20px', borderRadius: '14px', border: `1px solid ${T.border}`, minWidth: '180px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <span style={{ fontSize: '12px', color: T.textLight, fontWeight: '600' }}>Bid Limit</span>
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '8px' }}>
                        <span style={{ fontSize: '28px', fontWeight: '800', color: barColor }}>{totalBids}</span>
                        <span style={{ fontSize: '14px', color: T.textLight }}>/month</span>
                      </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}>
                          <div>
                            ...
                            <div style={{ display: 'flex', gap: '16px', fontSize: '13px', color: T.textMid, flexWrap: 'wrap' }}>
                              ...
                              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><MdCalendarToday size={13} color={T.textLight} /> {trip.start_date ? new Date(trip.start_date).toLocaleDateString('en-IN') : 'N/A'} → {trip.end_date ? new Date(trip.end_date).toLocaleDateString('en-IN') : 'N/A'}</span>
