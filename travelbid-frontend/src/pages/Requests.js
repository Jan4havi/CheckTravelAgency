...
const getDestImg = (d = '') => {
  ...
  if (s.includes('japan')||s.includes('tokyo')||s.includes('kyoto')) return 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?w=400&q=70';
...
export default function Requests() {
  ...
  const loadMyTrips = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from('trip_requests').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
      if (error) throw error;
      setLiveTrips((data || []).filter(t => t.status === 'Live'));
      setPreviousTrips((data || []).filter(t => t.status !== 'Live'));
    } catch (e) { console.error(e); } finally { setLoading(false); }
  };

  const loadUnlockedLeads = () => {
    setLoading(true);
    try { setUnlockedLeads(JSON.parse(localStorage.getItem('unlocked_leads') || '[]')); }
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '36px 32px', animation: 'fadeUp 0.5s ease' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '32px', fontWeight: '800', color: T.text, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            {isAgency ? <><MdLockOpen size={30} color={T.primary} /> My Lead Details</> : <><MdFlightTakeoff size={30} color={T.primary} /> My Trip Requests</>}
          </h1>
            ...
            {isAgency && (
              unlockedLeads.length > 0 ? (
                ...
                <div style={{ background: 'white', padding: '80px 40px', borderRadius: '20px', textAlign: 'center', border: `1px solid ${T.border}` }}>
                  ...
                  <button onClick={() => navigate('/bid-insights')} style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.primaryLight})`, color: 'white', border: 'none', padding: '12px 28px', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', margin: '0 auto' }}>
                    ...
                    <div key={trip.id} className="req-card" onClick={() => navigate(`/trip/${trip.id}`)}
                      ...
                      <div style={{ padding: '20px 24px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '10px' }}>
                          ...
                          <span style={{ background: '#dcfce7', color: '#166534', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '700' }}>Live</span>
                        </div>
                        <div style={{ display: 'flex', gap: '14px', fontSize: '13px', color: T.textMid, flexWrap: 'wrap', marginBottom: '12px' }}>
                          ...
                          <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><MdCalendarToday size={13} color={T.textLight} /> {trip.start_date}</span>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: `1px solid ${T.border}` }}>
                          <span style={{ fontSize: '12px', color: T.textLight, display: 'flex', alignItems: 'center', gap: '3px' }}><MdAccessTime size={13} /> {getPostedAgo(trip.created_at)}</span>
